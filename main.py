...
def main():
    cap = cv2.VideoCapture(
        0
    )  # "rtsp://admin:admin@192.168.88.97/media/video1") #"rtsp://admin:admin@192.168.88.97/media/video1"
    ...
    while cap.isOpened():
        ...
        if first_iteration_indicator == 1:
            ...
            ret, th1 = cv2.threshold(
                blur, threshold, maxValue, cv2.THRESH_BINARY + cv2.THRESH_OTSU
            )
            ...
            if (
                thDivider == FPS
            ):  # thus, we will get one frame each second. make 2*FPS if we want each two seconds for instance
                ...
                fileCounter5min += 1
                if (
                    time.time() - startOf5min > 60 * 5
                ):  # circular, up to 5 mins of images
                    fileCounter5min = 0
